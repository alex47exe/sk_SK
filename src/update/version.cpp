...
iSK_INI*
SK_Version_GetInstallIni (void)
{
  static iSK_INI install_ini (SK_Version_GetInstallIniPath ().c_str ());
                 install_ini.reload ();
...
iSK_INI*
SK_Version_GetRepoIni (void)
{
  static iSK_INI repo_ini (SK_Version_GetRepoIniPath ().c_str ());
                 repo_ini.reload ();
...
#pragma region ObsoleteVersionFetch1
bool
__stdcall
...
#define INJECTOR
  ...
  {
    iSK_INI& install_ini = *SK_Version_GetInstallIni ();
  ...
  InternetSetOptionW ( hInetGitLab, INTERNET_OPTION_RECEIVE_TIMEOUT, &ulTimeout, sizeof (ULONG) );

  SK_LOG0 ( (L"Fetching GitLab repository INI file from %s... ", wszRemoteRepoURL ),
             L"AutoUpdate" );
  ...
  HINTERNET hInetGitLabOpen =
    HttpOpenRequest ( hInetGitLab,
                        nullptr,
                          wszRemoteRepoURL,
                            L"HTTP/1.1",
                              nullptr,
  ...
  if (! hInetGitLabOpen)
  {
    InternetCloseHandle (hInetGitLab);
    InternetCloseHandle (hInetRoot);
    InterlockedExchange (&lock, 0);
    return false;
  }

  InternetSetOptionW ( hInetGitLabOpen, INTERNET_OPTION_RECEIVE_TIMEOUT, &ulTimeout, sizeof (ULONG) );
  ...
  //
  //////////////////////////////////////////////////////////////////////////
  if ( HttpSendRequestW ( hInetGitLabOpen,
                            nullptr,
                              0,
                                nullptr,
                                  0 ) )
  {
    ...
    if ( InternetQueryDataAvailable ( hInetGitLabOpen,
                                        &dwSize,
                                          0x00, NULL )
      )
    {
      ...
      while ((intptr_t)hVersionFile.m_h > 0 && dwSize > 0)
      {
        ...
        if ( InternetReadFile ( hInetGitLabOpen,
                                  pData,
                                    dwSize,
                                      &dwRead )
          )
    ...
    if (! bRet)
    {
      // GitLab may return 404: Not Found  (15 bytes)
  ...
  InternetCloseHandle (hInetGitLabOpen);
  InternetCloseHandle (hInetGitLab);
  InternetCloseHandle (hInetRoot) ;
...
bool
__stdcall
SK_FetchVersionInfo2 ( const wchar_t* wszProduct,
                             bool       bForce )
{
  //
  // Cache the last tested product
  ...
  // If no log is initialized yet, it's because SKIF is doing
  //   something with this DLL. Init. the log so we can trace
  //     install issues.
  if (! dll_log->initialized)
  {
    dll_log->init (
      L"logs/installer.log",
           L"wt+,ccs=UTF-8"
    );
  }
  ...
  if (wszProduct == nullptr)
  {   wszProduct =
    (! __SK_LastProductTested->empty ()) ?
       __SK_LastProductTested->c_str ()  : L"SpecialK";
  }

  __SK_LastProductTested.get () =
      wszProduct;
...
#define INJECTOR
  ...
  if (GetFileAttributes (SK_Version_GetInstallIniPath ().c_str ()) != INVALID_FILE_ATTRIBUTES)
  {
           ...
           install_ini.contains_section (L"Version.Local") )
    {
      ...
      static constexpr
        ULONGLONG _Hour = 36000000000ULL;
      ...
      else if (user_prefs.contains_key (L"Reminder"))
      {
        ...
        {   remind_time->register_to_ini (
          ...
          if ( remind_time->load ((LONGLONG&)remind) &&
                          uliNow.QuadPart >= remind )
          {
            need_remind = true;
          }
        }
                                  ...
                                  0 ) )
  {
    ...
    if (! bRet)
    {
      // May return 404: Not Found  (15 bytes)
      DeleteFileW (SK_Version_GetRepoIniPath ().c_str ());
    }
...
SK_Version_GetLatestInfo_V1 (const wchar_t* wszProduct)
{
  ...
  {
    iSK_INI& install_ini = *SK_Version_GetInstallIni ();
    iSK_INI& repo_ini    = *SK_Version_GetRepoIni    ();
